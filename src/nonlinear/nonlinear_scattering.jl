...
#=
    TO DO:
    -  have 1-to-1 functions as the LinearOptics case
    - Make Faster
=#
function get_intensity_over_an_angle(problem::NonLinearOptics{MeanField}, atoms_states::Vector, θ::Float64)
    ...
    intensity = (2π)*Γ^2/(4*k₀R^2)*real(sum(βₙₘ))
...
function _get_meanfield_constant_term(atoms, Θ)
    ...
    xⱼₘ, yⱼₘ, zⱼₘ = get_xyz_distances(r)
    k₀sinΘ = abs(k₀ * sin(Θ))
    cos_Θ = cos(Θ)
    ...
    @sync for n in 1:N
        Threads.@spawn for m in 1:N # we had to compute all terms, and not the upper part
            @inbounds Gₙₘ[n, m] = _constant_term_core_computation(xⱼₘ, yⱼₘ, zⱼₘ, n, m, cos_Θ, k₀sinΘ)
        end
    end
...
function _constant_term_core_computation(xⱼₘ, yⱼₘ, zⱼₘ, n, m, cos_Θ, k₀sinΘ)
    a = zero(ComplexF64)
    a = cis(k₀ * zⱼₘ[n, m] * cos_Θ) * besselj(0, k₀sinΘ * sqrt(xⱼₘ[n, m]^2 + yⱼₘ[n, m]^2))
    return a
end
@views function get_xyz_distances(r) # @memoize  --> creating warning, let's ignore it right now
    N = size(r, 2)

    xⱼₘ = Array{Float64,2}(undef, N, N)
    yⱼₘ = Array{Float64,2}(undef, N, N)
    zⱼₘ = Array{Float64,2}(undef, N, N)
