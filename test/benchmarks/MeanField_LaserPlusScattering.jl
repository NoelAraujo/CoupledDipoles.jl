...
# intensities
E = -(im/2)*Ω - (Γ/2)*(exp(im*k₀*r)/(im*k₀*r))*sum( σ⁻[j]*exp( -im*k₀*(n̂⋅R⃗[:,j])  ) for j=1:N)
I_1 = real(E*conj(E))

term1 = abs2(Ω)/4
term2 = real( im*Ω*(exp(-im*k₀*r)/(im*k₀*r))*sum(σ⁺[j]*exp( +im*k₀*(n̂⋅R⃗[:,j])  ) for j=1:N) )
term3 = sum(  σ⁻[j]*σ⁺[m]*exp(-im*k₀*(n̂⋅(R⃗[:,j]-R⃗[:,m])))  for j=1:N, m=1:N if j≠m)
term4 = sum(  (1 + σᶻ[j])/2 for j=1:N )
